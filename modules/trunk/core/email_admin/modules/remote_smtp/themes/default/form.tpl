<script type="text/javascript" src="/modules/smarthost/themes/js/javascript.js"></script>
...
<table class="tabForm" style="font-size: 16px;" COLS="4" width="100%" >
    ...
    <table border="0" width="100%" cellspacing="0" cellpadding="8">
        <tr class="letra12">
            ...
            <td align="left"><input type="radio" onclick="displayConfig();" value="e" id="status2" name="feature" align="left" tabindex="2"/>Enabled
            </td>
        </tr>

        <tr class="letra12">
            <td align="left"><b>{$host_name.LABEL}: <span class="required">*</span></b></td>
            <td align="left"> {$host_name.INPUT}</td>
        </tr>

        <tr class="letra12">
            {if $SMTP_server}
            <td align="left"><b>{$smtp_server.LABEL}: <span class="required">*</span></b></td>
            <td align="left">{$smtp_server.INPUT}</td>
            <input type="hidden" value="{$SMTP_SERVER}" name="tmpSmtp_server" />
            {/if}
...
<!--    {if $NewMaincf}
    ...
    <tr class="letra12">
        ...
        <td align="left"><b>{$broken_sasl_auth_clients.LABEL}: </b></td>
        <td align="left">{$broken_sasl_auth_clients.INPUT}</td>
        <input type="hidden" value="{$BROKEN_SASL_AUTH_CLIENTS}" name="tmpBroken_sasl_auth_clients" />
        {/if}
    </tr>-->
    ...
    <!--<tr class="letra12">
        ...
        <td align="left"><b>{$smtpd_sasl_auth_enable.LABEL}: </b></td>
        <td align="left">{$smtpd_sasl_auth_enable.INPUT}</td>
        <input type="hidden" value="{$SMTPD_SASL_AUTH_ENABLE}" name="tmpSmtpd_sasl_auth_enable" />
        {/if}

        {if $Smtp_always_send_ehlo}
        <td align="left"><b>{$smtp_always_send_ehlo.LABEL}: </b></td>
        <td align="left">{$smtp_always_send_ehlo.INPUT}</td>
        <input type="hidden" value="{$SMTPD_ALWAYS_SEND_EHLO}" name="tmpSmtp_always_send_ehlo" />
        {/if}
    </tr>

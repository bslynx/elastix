...
function viewFormEmailRelay($smarty, $module_name, $local_templates_dir, &$pDB, $arrConf, $arrLang)
{
    ...
    if(is_array($arr_MainCf) && count($arr_MainCf)>0){
        foreach($arr_MainCf as $key => $value){
            if($value['name']=="relayhost" || $value['name']=="#relayhost"){
                ...
                $arrData['relay_host'] = isset($_POST['relay_host'])?$_POST['relay_host']:$host[1];
                $smarty->assign("Relay_host", 1);
                $smarty->assign("RELAY_HOST", $value['value']);
                if(!empty($port[1])){
    ...
    $htmlForm = $oForm->fetchForm("$local_templates_dir/form.tpl",$arrLang["Smart Host"], $arrData);
...
function saveNewEmailRelay($smarty, $module_name, $local_templates_dir, &$pDB, $arrConf, $arrLang)
{
    ...
    if(!$oForm->validateForm($_POST)){
        ...
        $htmlForm = $oForm->fetchForm("$local_templates_dir/form.tpl",$arrLang["Smart Host"], $_POST);
        ...
        $pEmailRelay->replaceFileMainCF($host, $port, $data_step1, $result);
...
function createFieldForm($arrLang)
{
    ...
    $arrFields = array(
            "relay_host"   => array(      "LABEL"                  => $arrLang["Relay host"],
                                            "REQUIRED"               => "yes",
                                            "INPUT_TYPE"             => "TEXT",
                                            "INPUT_EXTRA_PARAM"      => "",

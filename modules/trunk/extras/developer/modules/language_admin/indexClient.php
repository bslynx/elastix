...
function _moduleContent(&$smarty, $module_name)
{
    ...
    switch($accion)
    {
        ...
        case "save_all":
            ...
            break;
        case "upload":
            $content = upload($smarty, $module_name, $local_templates_dir, $arrLang, $arrLangModule, $oForm);
            break;
...
function showLanguages($smarty, $module_name, $local_templates_dir, $arrLang, $arrLangModule)
{
    ...
    $smarty->assign("UPLOAD", $arrLangModule["Send translation to elastix.org"]);
...
function upload($smarty, $module_name, $local_templates_dir, $arrLang, $arrLangModule, $oForm)
{
    $oPalo = new paloSantoLanguageAdmin();
    $arrayLangTrasl = array();
    $module = getParameter("module");
    $language = getParameter("language");
    $arrayTmp = $_SESSION['arraytmp'];
    $arrayUp = array();
    $i = 0;
    foreach($_POST as $key => $value)
    {
    ...
    $bandera = $oPalo->upload($arrayLangTrasl, $module, $language, $arrayTmp, $arrayUp);
    if( !$bandera ){
        $smarty->assign("mb_title",$oPalo->errMsg['head'].":");
        $smarty->assign("mb_message",$oPalo->errMsg['body']);
    }

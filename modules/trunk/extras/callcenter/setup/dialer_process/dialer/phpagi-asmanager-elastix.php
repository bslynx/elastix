  ...
  */
  class AGI_AsteriskManager
  {
    ...
    */
    function send_request($action, $parameters=array())
    {
      ...
      foreach($parameters as $var=>$val)
        $req .= "$var: $val\r\n";
      $req .= "\r\n";
      $ret = fwrite($this->socket, $req);
      if ($ret) {
        return $this->wait_response();
      } else {
      	$this->request_err = TRUE;
        return NULL;
      }
    }
          ...
          if($a)
          {
            if(!count($parameters)) // first line in a response?
            {
              ...
              if(substr($buffer, $a + 2) == 'Follows')
              {
                ...
                if ($buff === FALSE) return NULL;
                while($buff != '' && substr($buff, 0, 6) != '--END ')
                {
    ...
    */
    ...
    function SetTimeout($iTimeoutSec, $iTimeoutUsec = 0)
    {
    	if (!is_null($this->socket)) stream_set_timeout($this->socket, $iTimeoutSec, $iTimeoutUsec);
    }
   ...
   // *********************************************************************************************************
    ...
    {
        if (is_object($o) && method_exists($o, 'output'))
            $this->oLogger = $o;
    }
    ...
    */
    function log($message, $level=1)
    {
      ...
      elseif (!is_null($this->oLogger))
        $this->oLogger->output($message);
      else

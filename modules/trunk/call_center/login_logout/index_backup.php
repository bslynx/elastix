...
function listadoLoginLogout($pDB, $smarty, $module_name, $local_templates_dir,&$oGrid,&$arrGrid,&$arrData) {
    ...
    }elseif(isset($_GET['txt_fecha_init']) ){
    ...
    if( isset($_POST['submit_fecha']) || isset($_POST['cbo_tipos'] )) {
        ...
        if ( (isset( $_POST['txt_fecha_init']) && $_POST['txt_fecha_init']!="")  ) {
        ...
        //PRUEBA
            ...
            if ( ereg( $sValidacion , $_GET['txt_fecha_init'] ) && ereg( $sValidacion , $_GET['txt_fecha_end'] )) {
                ...
                $arrFecha_init = explode('-',translateDate($fecha_end));
            }else {
                // si la fecha esta en un formato no valido se envia un mensaje de error
                $smarty->assign("mb_title", $arrLan["Error"]);
                $smarty->assign("mb_message", $arrLan["Debe ingresar una fecha valida"]);
...
//para el pagineo
       // LISTADO
        ...
        if(isset($_GET['nav']) && $_GET['nav']=="end") {
            $arrCallsTmp  = $oCalls->getRegistersLoginLogout($tipo,translateDate($fecha_init),$limit, $offset);
                ...
                <tr>
                    <td align='left'>
                        ...
                        <tr>
                            ...
                            <td>
                                ".insertarDateEnd($fecha_init_actual)."
                            </td>
                            <td class='letra12'>
                                &nbsp;
                            </td>

...
/*
    function crear_directorio($nombre_nuevo_directorio, $borrar_archivos, $respaldar, &$directorio_respaldo)
    {
        ...
        $usuario_grupo = $this->usuario_proceso.".".$this->usuario_proceso;
        $usuario_viejo = $this->usuario_sistema.".".$this->grupo_sistema;
        //echo "<br>chown ".$usuario_grupo." ".$this->directorio."<br>";

        if (!$this->privado_chown($usuario_grupo, $this->directorio)) {
            $mensaje = $this->errMsg;
            $this->errMsg = "Al cambiar permisos de directorio ".$nuevo_directorio .": $mensaje";
            return false;
        }

        if (!file_exists($nuevo_directorio)) {
            mkdir($nuevo_directorio);
            $this->privado_chown($usuario_viejo, $nuevo_directorio);
            //echo "chown $usuario_viejo $nuevo_directorio<br>";
            //echo "<script>alert('en crear dir 1');</script>";
        } else {
            if ($borrar_archivos) {
                $continuar = true;
                // Se cambia el dueño del directorio temporalmente a nobody.nobody para poder modificarlos
                if (!$this->privado_chown($usuario_grupo, $nuevo_directorio)) {
                    $mensaje = $this->errMsg;
                    // Deshacer los cambios que hayan alcanzado a suceder
                    $this->privado_chown($usuario_viejo, $nuevo_directorio);
                    $this->errMsg = "No han podido ser eliminados los archivos existentes. Al cambiar permisos de directorio " . 
                                        $nuevo_directorio .": $mensaje";
                    $continuar = false;
                }

                // Se cambia el dueno de los archivos que se van a modificar
                if ($continuar) {
                    $directorio=dir($nuevo_directorio);
                    if ($respaldar) {
                        $directorio_respaldo=$nuevo_directorio.date("Ymd").".".time();
                        //echo "==> $directorio_respaldo <br>";
                        mkdir($directorio_respaldo);
                        $this->privado_chown($usuario_grupo, $directorio_respaldo);
                    }
                    while ($archivo = $directorio->read()) {
                        $archivo_borrar = $nuevo_directorio."/".$archivo;
                        if ($this->privado_chown($usuario_grupo, $archivo_borrar)) {
                            if ($respaldar) {
                                //echo "new $directorio_respaldo/$archivo <br>";
                                //echo "old $archivo_borrar <br>";
                                if (is_file($archivo_borrar)) {
                                    copy($archivo_borrar, $directorio_respaldo."/".$archivo);
                                }
                            }
                    ...
                    $this->privado_chown($usuario_viejo, $nuevo_directorio);
                    //echo "chown $usuario_viejo $nuevo_directorio<br>";
                    //echo "<script>alert('en crear dir 2');</script>";
                }
            }
        }
        $this->privado_chown($usuario_viejo, $this->directorio);
        //echo "chown $usuario_viejo ".$this->directorio."<br>";
    ...
    {
        $continuar = true;
        $ruta_archivo=$this->directorio."/".$this->archivo;
        if(!file_exists($ruta_archivo)) {
            ...
            if ($this->privado_chown($this->usuario_proceso, $this->directorio)) {
                exec("sudo -u root touch $ruta_archivo");
        ...
        if ($continuar) {
            return $this->escribir_configuracion($arr_reemplazos,$overwrite);
        }
        return $continuar;
    }

    function crear_archivo_sin_establecer_permisos($archivo, $texto) 
    {
...
*/
    ...
    function establece_permisos($usuario, $grupo, $directorio) 
    {
        ...
        if (file_exists($directorio)) {
            $this->privado_chown($usuario_grupo, $directorio);
            //echo "chown $usuario_grupo $directorio<br>";
